"""Le cerveau LLM, avec un client Claude simule : on verifie le dossier
envoye, le decodage de la reponse, la comptabilite des couts, et les
chemins de repli (refus, JSON casse, plafond de depense)."""
...
from src.brains.llm_brain import OUTPUT_SCHEMA, LLMBrain, build_packet  # noqa: E402
from src.config import Config  # noqa: E402
from src.storage import Storage  # noqa: E402

CFG = Config(raw={
    "llm": {"model": "claude-opus-5", "effort": "medium", "max_tokens": 4000,
            "max_daily_api_cost_usd": 1.0, "price_input_per_mtok": 5.0, "price_output_per_mtok": 25.0},
...
def _ctx(positions=None):
    ...
    markets = {
        ...
        "ETH/USDT": MarketSnapshot("ETH/USDT", 10.0, df, {"rsi": 40.0, "trend": "down"}),
    }
    return BrainContext(
        cycle_id="c1", brain="llm", now_iso="2026-09-03T12:00:00+00:00", initial_capital=50.0,
        cash=50.0, positions=positions or [], markets=markets, recent_decisions=[], recent_trades=[],
        round_trips_used=1, round_trips_budget=3, daily_pnl_pct=0.0, fee_rate=0.001,
        limits={"max_position_pct": 0.4},
    )


def make_brain(response, monkeypatch=None):
    st = Storage(":memory:")
    b = LLMBrain(CFG, st)
...
def test_packet_contains_what_the_model_needs():
    p = build_packet(_ctx([PositionView(1, "BTC/USDT", 100.0, 0.2, 20.0, 119.0, "t", 92.0, 115.0)]))
    assert p["book"]["equity"] == 50.0 + 0.2 * 119.0
    assert p["limites"]["round_trips_restants_cette_semaine"] == 2
...
def test_decisions_are_decoded_sized_and_costed():
    resp = fake_response({
        ...
        "decisions": [
            {"symbol": "BTC/USDT", "action": "buy", "size_pct_of_equity": 0.9,
             "confidence": 0.7, "reasoning": "rebond sur EMA50"},
            {"symbol": "ETH/USDT", "action": "hold", "size_pct_of_equity": 0,
...
def test_daily_budget_stops_calls():
    brain, st = make_brain(fake_response({}))
    st.record_api_cost("claude-opus-5", 1, 1, 1.0)  # plafond atteint
    decs = brain.decide(_ctx())
    assert all(d.action == "hold" and d.raw.get("skipped") == "budget" for d in decs)
    assert brain._client.messages.calls == []  # aucun appel effectue
