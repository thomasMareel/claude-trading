...
def _ctx(cash=50.0, positions=None, rt_used=0, daily=0.0):
    snap = MarketSnapshot("BTC/USDT", 100.0, pd.DataFrame(), {})
    return BrainContext(
        cycle_id="t", brain="x", now_iso="now", initial_capital=50.0, cash=cash,
        positions=positions or [], markets={"BTC/USDT": snap, "ETH/USDT": snap},
        recent_decisions=[], recent_trades=[], round_trips_used=rt_used,
        round_trips_budget=3, daily_pnl_pct=daily, fee_rate=0.001,
    )


def _pos(symbol="BTC/USDT", entry=100.0, px=100.0, amount=0.2):
...
def test_buy_is_capped_at_max_position_pct():
    d, why = vet(_risk(), _ctx(cash=50.0), Decision("BTC/USDT", "buy", size_quote=45.0))
    assert why is None
    assert d.size_quote == 20.0  # 40% de 50
    assert d.raw.get("resized_from") == 45.0


def test_buy_below_minimum_is_rejected():
    d, why = vet(_risk(), _ctx(cash=50.0), Decision("BTC/USDT", "buy", size_quote=8.0))
    assert d is None and "minimum" in why


def test_buy_respects_exchange_min_notional():
    d, why = vet(_risk(), _ctx(cash=50.0), Decision("BTC/USDT", "buy", size_quote=15.0), min_notional=20.0)
    assert d is None and "minimum 20.00" in why


def test_no_pyramiding():
    ctx = _ctx(positions=[_pos()])
    d, why = vet(_risk(), ctx, Decision("BTC/USDT", "buy", size_quote=20.0))
...
def test_weekly_budget_counts_opens_this_cycle():
    d, why = vet(_risk(), _ctx(rt_used=2), Decision("BTC/USDT", "buy", size_quote=15.0), opens_this_cycle=1)
...
def test_buy_cannot_exceed_cash_after_earlier_buy():
    # cash_now reflete l'achat precedent du meme cycle
    d, why = vet(_risk(), _ctx(cash=50.0), Decision("ETH/USDT", "buy", size_quote=20.0), cash_now=10.0)
...
def test_kill_switch_is_sticky():
    r = _risk()
    assert r.kill_switch(80.0, 100.0) is False
    assert r.kill_switch(74.0, 100.0) is True
